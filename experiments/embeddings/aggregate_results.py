...
def create_comparison_table(aggregated_results: dict[str, dict]) -> Table:
    ...
    for config in sorted(aggregated_results.keys()):
        ...
        def format_metric(metric_key: str, metrics: dict) -> str:
        ...
        table.add_row(
            config,
            str(n_seeds),
            format_metric("knn_10", metrics),
            format_metric("knn_50", metrics),
            format_metric("spearman_r2", metrics),
            format_metric("pearson_r2", metrics),
            format_metric("nmi", metrics),
            format_metric("ari", metrics),
        )
...
def main(results_dir: Path | None = None) -> None:
    ...
    output_path = results_dir.parent / "embeddings_evaluation" / "aggregated_results.json"
