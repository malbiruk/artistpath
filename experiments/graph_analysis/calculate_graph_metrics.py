...
class StreamingGraphMetrics:
    ...
    def calculate_clustering_coefficient(
        ...
        node_sample: int = 200000,
    ) -> dict[str, Any]:
        """Calculate global clustering coefficient (transitivity) using sampling.

        Samples a fixed subset of nodes, then samples triplets from those nodes.
        This gives an unbiased estimate of clustering within the sampled subgraph.

        Note: node_sample should remain fixed. Vary sample_size to check convergence.
        """
        console.print("\n[cyan]Calculating clustering coefficient (sampled)...")

        # First pass: Collect node IDs and sample
        console.print(f"[cyan]Sampling {node_sample:,} nodes...")
        all_node_ids = []
        ...
        with self.graph_path.open() as f:
            for line in f:
                ...
                try:
                    ...
                    artist_id = data["id"]
                    connections = data["connections"][:MAX_EDGES_PER_NODE]
                    if connections:  # Only consider nodes with outgoing edges
                        all_node_ids.append(artist_id)
                except (json.JSONDecodeError, KeyError):
                    continue

        # Sample nodes
        sampled_nodes = set(random.sample(all_node_ids, min(node_sample, len(all_node_ids))))
        console.print(f"[green]✓ Sampled {len(sampled_nodes):,} nodes")

        # Second pass: Build adjacency structure ONLY for sampled nodes
        console.print("[cyan]Building adjacency structure for sampled nodes...")
        ...
        with self.graph_path.open() as f:
            for line in f:
                ...
                try:
                    ...
                    # Only load if this node was sampled
                    if artist_id not in sampled_nodes:
                        continue

                    connections = data["connections"][:MAX_EDGES_PER_NODE]
                    if connections:
                        neighbors = {
                            conn_id for conn_id, weight in connections if float(weight) > 0
                        }
                        if neighbors:
                            adjacency[artist_id] = neighbors
                            nodes_with_edges.append(artist_id)
                except (json.JSONDecodeError, KeyError):
                    continue

        console.print(f"[green]✓ Loaded adjacency for {len(sampled_nodes):,} sampled nodes")

        # Sample triplets and check for triangles (uniform node sampling)
        console.print(f"[cyan]Sampling up to {sample_size:,} triplets...")
        triangles = 0
        connected_triplets = 0
        attempts = 0
        max_attempts = sample_size * 10  # Try up to 10x to get enough valid triplets
        ...
        with Progress(
            ...
            task = progress.add_task("Checking triplets...", total=sample_size)

            while connected_triplets < sample_size and attempts < max_attempts:
                attempts += 1

                # Sample a random node uniformly
                if not nodes_with_edges:
                    break
                node_a = random.choice(nodes_with_edges)
                neighbors_a = adjacency[node_a]

                # Need at least 2 neighbors to form a triplet
                if len(neighbors_a) < 2:
                    continue

                # Sample two neighbors
                neighbors_list = list(neighbors_a)
                node_b, node_c = random.sample(neighbors_list, 2)

                # We have a connected triplet: A-B, A-C
                connected_triplets += 1

                # Check if B-C edge exists (triangle)
                if node_c in adjacency.get(node_b, set()) or node_b in adjacency.get(node_c, set()):
                    triangles += 1

                if connected_triplets % 10000 == 0:
                    progress.update(task, completed=connected_triplets)

        # Calculate clustering coefficient
        # Formula: triangles / connected_triplets (we sample center node + 2 neighbors)
        clustering_coef = triangles / connected_triplets if connected_triplets > 0 else 0.0

        console.print(
            f"[green]✓ Found {triangles:,} triangles out of {connected_triplets:,} connected triplets",
        )
        console.print(f"[green]✓ Clustering coefficient: {clustering_coef:.4f}")

        return {
            "clustering_coefficient": clustering_coef,
            "triangles_sampled": triangles,
            "triplets_sampled": connected_triplets,
    ...
    def get_top_nodes(self, n: int = 20) -> dict[str, list[tuple[str, int]]]:
        ...
        # Get top by out-degree (deduplicate by node_id)
        seen_ids = set()
        top_out_deduped = []
        for deg, node_id in self.top_out_degrees:
            if node_id not in seen_ids:
                top_out_deduped.append((deg, node_id))
                seen_ids.add(node_id)
            if len(top_out_deduped) >= n:
                break
        top_out_ids = [node_id for _, node_id in top_out_deduped]
        ...
        return {
            ...
            "top_by_out_degree": [(node_names.get(nid, nid), deg) for deg, nid in top_out_deduped],
        }
...
def main() -> None:  # noqa: PLR0915
    ...
    # Calculate clustering coefficient (requires second pass with adjacency)
    clustering_stats = metrics.calculate_clustering_coefficient()
    ...
    table.add_row(
        "Clustering coef.",
        f"{clustering_stats['clustering_coefficient']:.4f}",
    )
    table.add_row(
        "Out-degree (mean/med)",
