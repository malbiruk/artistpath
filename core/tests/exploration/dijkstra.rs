...
fn test_dijkstra_basic_exploration() {
    ...
    assert!(result.discovered_artists.contains_key(&test_graph.taylor_id));
...
fn test_dijkstra_similarity_filtering() {
    ...
    for (_, (similarity, _)) in &result.discovered_artists {
        if *similarity > 0.0 {  // Skip the starting artist which has similarity 1.0
...
fn test_dijkstra_finds_closest_artists_by_weight() {
    ...
    assert!(result.discovered_artists.len() >= 1);
    assert!(result.discovered_artists.contains_key(&test_graph.taylor_id));
...
fn test_dijkstra_layer_assignment() {
    ...
    let layer_0_artists: Vec<_> = result.discovered_artists
        .iter()
        .filter(|(_, (_, layer))| *layer == 0)
        .map(|(id, _)| *id)
        .collect();
    
    assert_eq!(layer_0_artists.len(), 1, "Only one artist should have layer 0");
    assert_eq!(layer_0_artists[0], test_graph.taylor_id, "Center artist should have layer 0");
    
    // All other artists should have layer > 0
    for (&artist_id, &(_, layer)) in &result.discovered_artists {
        if artist_id != test_graph.taylor_id {
            assert!(layer > 0, "Non-center artist {:?} should have layer > 0, got {}", artist_id, layer);
        }
    }
}