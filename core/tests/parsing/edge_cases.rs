...
fn test_find_artist_fuzzy_match() {
    ...
    lookup.insert("taylor swift".to_string(), vec![id1]);
    lookup.insert("the beatles".to_string(), vec![id2]);
...
fn test_find_artist_unicode_normalization() {
    ...
    lookup.insert("bjork".to_string(), vec![id]);
...
fn test_find_artist_not_found_message() {
    ...
    assert!(
        result
            .unwrap_err()
            .contains("Artist 'Nonexistent Artist' not found")
    );
}
...
fn test_find_artist_empty_name() {
    let mut lookup = FxHashMap::default();
    let id = Uuid::new_v4();
    lookup.insert("test".to_string(), vec![id]);
